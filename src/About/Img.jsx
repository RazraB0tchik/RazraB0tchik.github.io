...
const Img = (props) => {
    ...
    // let scrollBar = document.getElementsByClassName("block_with_pictures")[0];
    //
    // console.log(scrollBar.scrollWidth + 'scroll');
    //
    // scrollBar.scrollWidth = picturesHight.length * 450;
    //
    //
    // console.log(props.position +  ' now_position')

    const[activeModul, setActiveModul] = useState(false);
    // function displacementElem() {
    //     console.log(-picturesHight.length*2 * distance + ' distance change')
    //     return -picturesHight.length*2 * distance;
    // }
    //
    // function displacementElemNewPos() {
    //     console.log(-picturesHight.length * distance * 2 + ' distance change')
    //     return -picturesHight.length * distance * 2;
    // }
    ...
    const setParams = () =>{
        ...
        if(window.innerWidth>800) {
            ...
            // SetImg_St(picturesHight[pictureId].img);
    }
    }
    ...
    const clickR = () => {
        if(pictureId<picturesHight.length) {
            ...
            // SetImg_St(picturesHight.find(elem=>elem.id === pictureId));
            picturesHight.forEach(elem => {
                if (elem.id === picture) {
                    return SetImg_St(elem.img);
                }
    ...
    const clickL = () => {
        if(pictureId>1) {
            let picture = pictureId - 1;
            setPictureId(picture);
            console.log(pictureId + 'ckL')
            // SetImg_St(picturesHight.find(elem=>elem.id === pictureId).img);
            picturesHight.forEach(elem => {
                if (elem.id === picture) {
                    return SetImg_St(elem.img);
                }
    ...
    //     if (props.flagScroll === false){
    //         if(props.position===undefined) {
    //             return picturesHight.length * 2 * distance / timeZone;
    //         }
    //         else{
    //             return (picturesHight.length*distance*2) / timeZone;
    //         }
    //     }
    //     else{
    //         return 9999999;
    ...
    // useEffect(()=>{
    //     if(picture.onmousedown){
    //         console.log('нажал')
    //     }
    // }, [picture])

    // initial={{x:0}} animate={{x: props.position===undefined ? displacementElem() : displacementElemNewPos()}} transition={{duration: checkPositionUser(), repeat: Infinity, ease: "linear"}}
